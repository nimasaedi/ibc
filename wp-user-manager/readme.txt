...
For more advanced functionality check out the [pricing page](https://wpusermanager.com/pricing/?utm_source=wordpress.org&utm_medium=free%20plugin%20listing&utm_campaign=WP%20User%20Manager).
...
- Security: Missing authorization to authenticated check for the unique usermeta key check when editing a field (Thanks to Tieu Pham Trong Nhan)
- Security: Carbon Field sidebar AJAX action handlers removed which are not used but posed a CSRF security risk (Thanks to BrokenAC)

= 2.9.11 (7th Sept 2024) =

- Security: Nonce check added to the admin action of fixing old plugin data

= 2.9.10 (2nd April 2024) =

- Improvement: + and other characters no longer accepted in username at registration
- Fix: Required checkboxes field hidden with conditional logic incorrectly requires all boxes to be checked
- Fix: Registration form duplicate id attribute HTML issues
...
= Earlier versions =
For the full changelog with earlier versions, please refer to [the changelog on wpusermanager.com](https://wpusermanager.com/changelog/).
